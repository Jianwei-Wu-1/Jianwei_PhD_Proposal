...
Finally, I plan to propose an innovative, class under test (CUT)'s JavaDoc-based approach to investigate if we can use a set of pre-mined JavaDoc patterns to extract the descriptive information from the JavaDocs of the associated classes under test (CUTs) of each test for deciding if the test is named after that information from its CUT JavaDocs.
%
During our preliminary study, we found out a change of code elements in the test body of a test or a rename of the test seldom facilitates a corresponding change in the test name.
%
On the contrary, the JavaDocs of classes under test does provide descriptive information that can be used to fill the gap when the uniqueness-based naming rationale is not applicable in certain conditions.
%
